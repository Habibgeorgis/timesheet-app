...
export default async function DashboardPage() {
  const user=await requireUser(); if(user.role===Role.MANAGER||user.role===Role.ADMIN)redirect("/manager"); const weekStart=mondayOf(new Date()); const current=await ensureTimesheet(weekStart,user.id);
  ...
  <section className="grid gap-4 sm:grid-cols-3"><Metric icon={Clock3} label="This week" value={formatDuration(currentMinutes)} detail={`${Math.round(currentMinutes/24)/10}% of 40h`}/><Metric icon={CalendarDays} label="Current week" value={weekLabel(weekStart)} detail="Monday through Friday"/><Metric icon={CalendarClock} label="Days tracked" value={String(trackedDays)} detail="Last 8 weeks"/></section>

...
export default async function ManagerPage({searchParams}:{searchParams:Promise<{week?:string}>}){
  ...
  const requestedWeek=mondayOf(week&&/^\d{4}-\d{2}-\d{2}$/.test(week)?new Date(`${week}T12:00:00Z`):currentWeek);
  const selectedWeek=dateKey(requestedWeek)>dateKey(currentWeek)?currentWeek:requestedWeek;
  const isCurrentWeek=dateKey(selectedWeek)===dateKey(currentWeek);

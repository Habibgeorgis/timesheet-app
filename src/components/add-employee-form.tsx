...
export function AddEmployeeForm(){
  ...
  return <details className="relative">
    ...
    <form action={action} className="panel absolute right-0 z-20 mt-2 w-[min(380px,calc(100vw-32px))] space-y-4 p-5 shadow-xl" noValidate>
      ...
      <Field label="Employee ID" name="employeeCode" value={values.employeeCode} onChange={update("employeeCode")} error={state.fieldErrors?.employeeCode}/>
      <Field label="Job title" name="jobTitle" value={values.jobTitle} onChange={update("jobTitle")} error={state.fieldErrors?.jobTitle}/>
...
function Field({label,name,type="text",value,onChange,error}:{label:string;name:string;type?:string;value:string;onChange:(event:React.ChangeEvent<HTMLInputElement>)=>void;error?:string}){return <div><label className="label" htmlFor={`employee-${name}`}>{label}</label><input className={cn("input",error&&"input-error")} id={`employee-${name}`} name={name} type={type} value={value} onChange={onChange} autoComplete={name}/>{error&&<p role="alert" className="mt-1.5 text-xs font-semibold text-red-600">{error}</p>}</div>}

...
export function AppShell({user,children}:{user:User;children:React.ReactNode}) {
  ...
  const links=manager?[{href:"/manager",label:"Team hours",icon:Users}]:employeeLinks;
  ...
  return <div className="min-h-screen md:grid md:grid-cols-[232px_1fr]">
    <aside className="desktop-only fixed inset-y-0 left-0 z-20 flex w-[232px] flex-col bg-[#102a26] px-3 py-5 text-white">
      <Link href={home} className="mb-8 flex items-center gap-3 px-3 text-xl font-bold"><span className="grid size-9 place-items-center rounded-md bg-[#22a98f]"><Clock3 size={20}/></span>Tempo</Link>
    ...
    <div className="md:col-start-2"><header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-[#dce3e0] bg-white px-4 md:px-8"><Link href={home} className="flex items-center gap-2 font-bold md:hidden"><Clock3 className="text-[#087f6b]"/>Tempo</Link><div className="desktop-only text-sm text-[#66736f]">{manager?"Team hours workspace":"Time tracking workspace"}</div><div className="flex items-center gap-3 text-sm font-semibold"><span className="hidden sm:inline">{user.name}</span><span className="grid size-8 place-items-center rounded-full bg-[#d8efe9] text-xs text-[#075f51]">{initials(user.name)}</span></div></header><main className="mx-auto max-w-[1280px] p-4 pb-24 md:p-8">{children}</main>
